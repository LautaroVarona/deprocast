...
export function TrincheraDayFocus({ onRefreshSnapshot }: TrincheraDayFocusProps) {
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col overflow-hidden border-r border-border/[0.06] bg-background">
      <header className="border-b border-border/[0.06] px-4 py-3">
        <h2 className="text-sm font-semibold text-foreground">Foco diario</h2>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-y-auto p-4 xl:grid-cols-2">
          <article className="rounded-xl border border-border bg-muted/40 p-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <h3 className="text-sm font-medium text-foreground">Obligatorias</h3>
            </div>
            <ul className="space-y-2">
              {requiredTasks.length === 0 ? (
                <li className="text-xs text-muted-foreground">No hay tareas obligatorias para este día.</li>
              ) : (
                requiredTasks.map((task) => (
                  <li key={task.id} className="rounded-md border border-border bg-card/80 px-3 py-2">
          ...
          <article className="rounded-xl border border-border bg-muted/40 p-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <h3 className="text-sm font-medium text-foreground">Sugeridas</h3>
            </div>
            <ul className="space-y-2">
              {suggestedTasks.length === 0 ? (
                <li className="text-xs text-muted-foreground">No hay sugerencias para este día.</li>
              ) : (
                suggestedTasks.map((task) => (
                  <li key={task.id} className="rounded-md border border-border bg-card/80 px-3 py-2">
                    <p className="text-sm text-foreground">{task.title}</p>
                    <p className="font-mono text-[10px] text-muted-foreground">{task.source}</p>
                  </li>
                ))
              )}
            </ul>
          ...
          <article className="xl:col-span-2 rounded-xl border border-border bg-muted/40 p-3">
            <h3 className="mb-2 text-sm font-medium text-foreground">Asaltos disponibles</h3>
            <div className="grid gap-2 md:grid-cols-2">
              {asaltos.length === 0 ? (
                <p className="text-xs text-muted-foreground">Sin asaltos. Calibrá tareas en Pendientes.</p>
              ) : (
                asaltos.map((asalto) => (

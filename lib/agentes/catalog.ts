...
export const OPERATIONAL_AGENTS: OperationalAgent[] = [
  ...
  {
    id: "hermeneuta-cuadernos",
    emoji: "📜",
    name: "Hermeneuta de Cuadernos",
    badge: "Extractor Visual · Cohere Vision",
    badgeTone: "violet",
    locations: [
      "lib/hermeneuta/extract.ts",
      "lib/hermeneuta/prompts.ts",
      "app/api/hermeneuta/route.ts",
      "components/hermeneuta/panal-workspace.tsx",
      "app/cam-recorder/page.tsx",
    ],
    functions: [
      "Recibir imagen de cuaderno (WebP/PNG/JPG) vía FormData — preferible comprimida en cliente.",
      "Invocar Cohere Vision con system prompt de transcripción literal (Vector Semántico).",
      "Descifrar caligrafía manuscrita al pie de la letra sin inventar contenido.",
      "Devolver semanticText crudo para triage HITL — jamás persistir sin coagulación humana.",
      "Alimentar el Espejo del Panal de Ingesta Visual (/cam-recorder).",
    ],
    technologies: [
      "Cohere Command A Vision",
      "lib/cohere/vision.ts",
      "Soberanía local · HITL estricto",
    ],
    uiRoute: "/cam-recorder",
  },
  {
    id: "mapeador-simbolico",
    emoji: "⟁",
    name: "Mapeador Simbólico",
    badge: "Alquimista Relacional · Vision",
    badgeTone: "amber",
    locations: [
      "lib/hermeneuta/extract.ts",
      "lib/hermeneuta/prompts.ts",
      "lib/hermeneuta/coagulate.ts",
      "app/api/hermeneuta/route.ts",
      "app/api/hermeneuta/coagulate/route.ts",
    ],
    functions: [
      "Analizar la misma imagen buscando el Vector Estructural (flechas, viñetas, círculos, llaves).",
      "Proponer entidades (personas, proyectos, conceptos) y relaciones tipadas de base de datos.",
      "Devolver JSON estricto structuralNodes con nodos y aristas sugeridas (ej. A → depende_de → B).",
      "Peso hermético default 6; coagulación solo tras toggle HITL y botón COAGULAR EN EL GRAFO.",
      "Inyectar KgNode/KgEdge con reconocido: true únicamente tras validación del operador.",
    ],
    technologies: [
      "Cohere Command A Vision (JSON)",
      "Prisma (KgNode, KgEdge)",
      "lib/kg/ingest.ts",
  ...
  },
  {
    ...
    emoji: "⚙️",
    name: "Task-Breaker",
    badge: "Orquestador · Alquimista",
    badgeTone: "amber",
    locations: [
      "lib/ludus/task-breaker.ts",
      "app/api/ludus/task-breaker/route.ts",
      "app/api/ludus/task-breaker/coagulate/route.ts",
      "components/ludus/trituradora/trituradora-workspace.tsx",
      "app/ludus/trituradora/page.tsx",
    ],
    functions: [
      "Recibir un Project, entidades (personas/contextos) y un objetivo narrativo.",
      "Llamar a Cohere para fragmentar el input en JSON estricto de micro-tareas secuenciales.",
      "Garantizar que ninguna tarea supere 40 minutos (óptimo 15–25).",
      "Proponer micro-tareas en memoria; coagulación HITL hacia PendingTask reconocida.",
      "Conectar projectId para que las tareas aparezcan en Trinchera y Campamento.",
    ],
    technologies: [
      ...
      "lib/pendientes/store.ts",
      "SQLite local-first",
      "HITL (propuesta ≠ persistencia)",
    ],
    uiRoute: "/ludus/trituradora",
  },
  {
    id: "task-calibrator",
    emoji: "🎯",
  ...
  },
  {
    ...
    functions: [
      "Legado: ingerir screen recordings (.mp4, .m4v, .mov, .webm, .mkv) vía drag-and-drop.",
      "La ruta /cam-recorder ahora hospeda el Panal de Ingesta Visual (Hermeneuta + Mapeador).",
      "API y componentes mock conservados para auditoría Molecular / Variable X.",
    ],
    technologies: [
  ...
  },
  {
    ...
    emoji: "🧲",
    name: "Meta-Meteador",
    badge: "Alquimista · Gravedad semántica 1–12",
    badgeTone: "violet",
    locations: [
      "lib/kg/quantomo-gravity.ts",
      ...
      "app/api/kg/graphrag/route.ts",
      "components/agentes/meta-meteador-panel.tsx",
      "components/agentes/meta-meteador-modal.tsx",
    ],
    functions: [
      "Calcular distancia semántica (cosine) entre dos Quántomos aislados vía embeddings Cohere.",
      "Proponer KgEdge con peso hermético 1–12 si superan el umbral de similitud (reconocido: false hasta HITL).",
      "Mapear similitud [0,1] → Escala Hermética de Gravedad sin inventar aristas coaguladas.",
      "Revisar documentos validados (.md en data/projects) en lote (legado DocumentMeta).",
    ...
    technologies: [
      "Cohere embed-v4.0",
      "lib/mnemosyne/cosine (JS puro)",
      "Prisma (Quantomo, KgEdge, DocumentMeta)",
      "lib/validations/kg-schema (weight 1–12)",
      "SQLite local-first",
    ],
    uiRoute: "/grafo",
  },
  {
    ...
    badgeTone: "emerald",
    locations: [
      "lib/personas/prosopografo/prompt.ts",
      "lib/personas/prosopografo/parse.ts",
      "lib/personas/prosopografo/import.ts",
      "lib/personas/prosopografo/resolve-connections.ts",
      "app/api/agentes/prosopografo/prompt/route.ts",
      "app/api/agentes/prosopografo/import/route.ts",
      "components/agentes/prosopografo-workspace.tsx",
      "app/agentes/prosopografo/page.tsx",
    ],
    functions: [
      "Emitir prompt-cuestionario Matriz CRM 6×6 para LLMs externos (Gemini, etc.).",
      "Copiar / descargar el cuestionario desde la UI del agente o desde /personas.",
      "Parsear JSON pegado o subido (objeto, array o { personas: [] }).",
      "Resolver connectionsByName a nodos KgNode existentes; omitir con warning si no hay match.",
      "Coagular personas verificadas vía createPersonaWithRelations (reconocido: true).",
      "Sellar en universo Babel activo y refrescar dominio personas/grafo.",
    ],
    technologies: [
      "Zod",
      "PersonaCrmModules (metadata.crm)",
      "createPersonaWithRelations",
      "Prisma (KgNode)",
      "HITL (sin LLM interno)",
    ],
    uiRoute: "/agentes/prosopografo",
  },
  {
    id: "binauralizer",
    emoji: "🎧",
  ...
  },
  {
    ...
    emoji: "⚛️",
    name: "Quantador",
    badge: "Extractor · Embeddings Cohere",
    badgeTone: "violet",
    locations: [
    ...
    functions: [
      "Trocear materia prima en Quántomos moleculares indexables (título, tags, universo).",
      "Pasar cada Quántomo por Cohere embed-v4 y persistir el vector JSON en Quantomo.embedding.",
      "Espejar el Quántomo como KgNode (type=quantomo) para que el GraphRAG viaje por KgEdge.",
    ...
    technologies: [
      "Cohere Command R+ (JSON)",
      "Cohere embed-v4.0",
      "Prisma (Quantomo, OriginAttribution, KgNode)",
  ...
  },
  {
    ...
    emoji: "🕳",
    name: "Motor GraphRAG",
    badge: "Orquestador · Pensadero",
    badgeTone: "amber",
    locations: [
      "lib/kg/graphrag.ts",
      "lib/kg/quantomo-gravity.ts",
      "app/api/kg/graphrag/route.ts",
      "components/grafo/pensadero-workspace.tsx",
      "app/grafo/page.tsx",
    ],
    functions: [
      "Vectorizar la query del usuario vía Cohere embed (search_query).",
      "Hallar los 5 Quántomos más afines por similitud coseno en memoria (JS puro sobre SQLite).",
      "Viajar por KgEdge con reconocido==true y gravedad alta (>8) para iluminar la Órbita.",
      "Devolver Zona de Impacto: Core (match directo) + Órbita (confirmados + sugeridos HITL).",
      "Nunca coagular aristas solo: las conexiones semánticas en caliente se ofrecen al 50% de opacidad.",
    ],
    technologies: [
      "Cohere embed-v4.0",
      "lib/mnemosyne/cosine",
      "Prisma (Quantomo, KgNode, KgEdge)",
      "SQLite local-first · egress solo Cohere",
    ],
    uiRoute: "/grafo",
  },
  {
  ...
  {
    id: "destilador-jornada",
    emoji: "🧪",
    name: "Destilador de Jornada",
    badge: "Draft · Atanor",
    badgeTone: "amber",
    locations: [
      "lib/jornada/actions.ts",
      "lib/jornada/chat-engine.ts",
      "lib/jornada/destilador.ts",
      "components/jornada/atanor-workspace.tsx",
    ],
    functions: [
      "Copiloto Cohere en hilos vivos del día (chat del Atanor).",
      "Leer todos los hilos paralelos de una DailySession al cerrar.",
      "Extraer promesas, decisiones y contexto (tags RAG).",
      "Coagular un documento Markdown estructurado para el Corpus de Deprocast vía Cohere.",
    ],
    technologies: [
      "Prisma (DailySession, SessionThread, ThreadMessage, ThreadContext)",
      "Server Actions",
      "Cohere (cohereGenerateText chat vivo + cohereGenerateJson coagulación)",
    ],
    uiRoute: "/jornada",
  },
  {
